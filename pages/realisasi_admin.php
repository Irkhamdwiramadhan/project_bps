...
$daftar_tahun = [];
// Query untuk mengambil daftar tahun unik dari master_item dan rpd (logika gabungan)
...
// Jika tidak ada tahun yang ditemukan, set tahun filter ke null
if (empty($daftar_tahun)) {
    $tahun_filter = null;
} elseif (!in_array($tahun_filter, $daftar_tahun)) {
    // Jika tahun yang difilter tidak ada di daftar tahun, gunakan tahun terbaru
    $tahun_filter = $daftar_tahun[0];
}


// Bangun kueri dan parameter secara kondisional hanya jika tahun filter tersedia
$data_anggaran = [];
if ($tahun_filter !== null) {
    // Kueri lengkap untuk semua peran (logika gabungan)
    $sql = "SELECT
            mi.id AS id_item,
            mi.nama_item,
            mi.pagu,
        ...
        WHERE mi.tahun = ? 
        GROUP BY mi.id ORDER BY mk.nama, ma.nama, mi.nama_item ASC";
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            $sql_detail = "SELECT
                                            ...
                                            GROUP BY rpd.bulan ORDER BY rpd.bulan ASC";
                            ...
                            for ($bulan = 1; $bulan <= 12; $bulan++):
                                $jumlah_rpd = $data_bulan[$bulan]['rpd'] ?? 0;
                                $jumlah_realisasi = $data_bulan[$bulan]['realisasi'] ?? 0;
                            ?>
                                <td class="text-primary"><?= number_format($jumlah_rpd, 0, ',', '.') ?></td>
                                <td class="text-success"><?= number_format($jumlah_realisasi, 0, ',', '.') ?></td>
                            <?php endfor; ?>
                        ...
                        <?php else: ?>
                        <tr>
                            <td colspan="27" class="text-center text-muted">Tidak ada data anggaran yang ditemukan untuk tahun ini.</td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
