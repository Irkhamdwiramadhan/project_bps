...
<div class="main-content">
    ...
    <div class="card">
        ...
        <div id="loading-indicator" style="display:none; text-align:center; padding:30px; color:#6b7280;">
            <svg style="width:24px; height:24px; animation:spin 1s linear infinite; margin-bottom:10px;" fill="none" viewBox="0 0 24 24">
        ...
        <div id="mitra-list-container" class="table-responsive" style="display:none;">
            <table class="custom-table" id="tabel-mitra">
                <thead>
                    <tr>
                        <th>Nama Mitra</th>
                        ...
                        <th style="min-width: 150px;">
                            <div style="display:flex; align-items:center; justify-content:space-between;">
                                Status
                                <select id="filter_status_row" class="header-filter-select">
        ...
        <div class="modal-body">
            ...
            <div class="col-guide">
                ...
                <table class="guide-table">
                    <tr>
                        <th width="40">4</th>
                        <td><span class="val-good">Sangat Baik</span> (Benar 100%)</td>
                    </tr>
                    <tr>
                        <th>3</th>
                        <td>Baik (Benar 70% - 90%)</td>
                    </tr>
                    <tr>
                        <th>2</th>
                        <td>Cukup (Benar 50% - 60%)</td>
                    </tr>
                    <tr>
                        <th>1</th>
                        <td><span class="val-bad">Kurang</span> (Di bawah 50%)</td>
                    </tr>
                </table>
                <div class="guide-title">ℹ️ Panduan: Volume</div>
                <table class="guide-table">
                    <tr>
                        <th width="40">4</th>
                        <td><span class="val-good">2 Hari Sebelum Deadline</span></td>
                    </tr>
                    <tr>
                        <th>3</th>
                        <td>Tepat di Hari Deadline</td>
                    </tr>
                    <tr>
                        <th>2</th>
                        <td>Terlambat (Volume Sesuai)</td>
                    </tr>
                    <tr>
                        <th>1</th>
                        <td><span class="val-bad">Terlambat (Volume Tidak Sesuai)</span></td>
                    </tr>
                </table>
                ...
                <table class="guide-table">
                    <tr>
                        <th width="40">4</th>
                        <td><span class="val-good">Sangat Memuaskan</span></td>
                    </tr>
                    <tr>
                        <th>3</th>
                        <td>Memuaskan</td>
                    ...
                    <tr>
                        <th>1</th>
                        <td><span class="val-bad">Tidak Memuaskan</span></td>
                    </tr>
                </table>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Elemen Filter Status Baru
        const filterStatus = document.getElementById('filter_status_row');
            ...
            fetch(`get_status_penilaian_tim.php?tim_id=${timId}&tahun=${tahun}&_=${Date.now()}`)
                ...
                .then(data => {
                    ...
                    data.forEach(item => {
                        ...
                        // Menambahkan kelas status ke TR untuk mempermudah filtering
                        const rowStatusClass = item.penilaian_id ? 'status-sudah' : 'status-belum';
                    ...
                    // Terapkan filter status setelah data dimuat (agar status tersimpan)
                    applyStatusFilter();
                })
                .catch(err => {
                    console.error(err);
        ...
        function applyStatusFilter() {
            const status = filterStatus.value; // all, sudah, belum
            ...
            rows.forEach(row => {
                if (status === 'all') {
                    ...
                    row.style.display = row.classList.contains('status-sudah') ? '' : 'none';
                } else if (status === 'belum') {
                    row.style.display = row.classList.contains('status-belum') ? '' : 'none';
                }
            });
        }
        ...
        filterStatus.addEventListener('change', applyStatusFilter); // Listener baru
        ...
        if (filterTim.value) loadDataMitra();
    });
</script>
