...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            $kode = preg_replace('/[\x00-\x1F\x7F\xA0]/u', '', $kode);
            ...
            if (preg_match('/^\d{3}\.\d{2}\.[A-Z]{2}$/', $kode)) {
                ...
                // === ITEM PROCESSING BLOCK (FIXED) ===

                // Gunakan getCalculatedValue() untuk keamanan jika ada rumus
                $raw_volume_satuan = trim((string)$sheet->getCell('J' . $row)->getCalculatedValue());
                $raw_harga         = $sheet->getCell('K' . $row)->getCalculatedValue();
                $raw_pagu          = $sheet->getCell('L' . $row)->getCalculatedValue();
                ...
                // Helper local untuk membersihkan angka (menggantikan filter_var SANITIZE_NUMBER_INT yg bug)
                $clean_number = function($val) {
                    if (is_numeric($val)) return (float)$val;
                    // Bersihkan karakter non-numeric, ganti koma jadi titik
                    $val = str_replace(['Rp', ' ', '.'], '', (string)$val);
                    $val = str_replace(',', '.', $val);
                    return (float)$val;
                };

                $harga = (int)($clean_number($raw_harga) * 1000);
                $pagu  = (int)($clean_number($raw_pagu) * 1000);
                ...
                if ($is_continuation) {
                    ...
                    if ($stmtGet === false) throw new Exception("Prepare failed: " . $koneksi->error);
                    ...
                    if ($existingRow = $resGet->fetch_assoc()) {
                        ...
                        if ($stmtUpd === false) throw new Exception("Prepare failed: " . $koneksi->error);
                ...
                foreach ($monthly_map as $bulan => $cols) {
                    ...
                    if (is_array($cols)) {
                        foreach ($cols as $col) {
                            ...
                            $jumlah_rpd += $clean_number($rawVal);
                        }
                    } else {
                        $rawVal = $sheet->getCell($cols . $row)->getCalculatedValue();
                        $jumlah_rpd = $clean_number($rawVal);
                    }
                    $jumlah_rpd = $jumlah_rpd * 1000;
